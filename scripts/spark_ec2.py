...
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key, copyfiles):
    ...
    tmpuser = opts.user
    opts.user = "ec2-user"
    ssh(master, opts, """sudo cp -f /home/ec2-user/.ssh/authorized_keys /root/.ssh""")
    opts.user = tmpuser
    if deploy_ssh_key:
        ...
        ssh(master, opts, """rm -f /root/.ssh/known_hosts""")
        ...
        for slave in slave_nodes:
            ...
            tmpuser = opts.user
            opts.user = "ec2-user"
            ssh(slave_address, opts, """sudo cp -f /home/ec2-user/.ssh/authorized_keys /root/.ssh""")
            opts.user = tmpuser
            ssh_write(slave_address, opts, ['tar', 'x'], dot_ssh_tar)
            ssh(slave_address, opts, """rm -f /root/.ssh/known_hosts""")
...
def setup_spark_cluster(master, opts, copyfiles):
